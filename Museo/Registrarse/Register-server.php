...
if (isset($_POST['submitUp'])) { 
        if (strlen($_POST['name']) > 1 && strlen($_POST['email']) > 1 && strlen($_POST['contraseña']) > 1) { //verifica que los campos no esten vacios
            ...
            if (empty($Nombre_Imagen)) {
                ...
                $result = mysqli_query($conex, "SELECT * FROM `login` WHERE Email='$email'");
                if (mysqli_num_rows($result) > 0)  // Si es mayor a cero imprimimos que ya existe el usuario
                {
                    ...
                    $Pedido = "INSERT INTO `login`(`Nombre`, `Email`, `Imagen`, `Contraseña`) VALUES ('$name','$email','$Imagen_Default','$contraseña')";
                    $Resultado = mysqli_query($conex, $Pedido);
                    if ($Resultado) { //verifica que los datos se envien a la base de datos
                        echo "<h>Te inscribiste correctamente</h>";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $Pedido = "INSERT INTO `login`(`Nombre`, `Email`, `Imagen`, `Contraseña`) VALUES ('$name','$email','./intranet/uploads/$Nombre_Imagen','$contraseña')"; //Inserta todos los datos a la base de datos
                    $Resultado = mysqli_query($conex, $Pedido);
                    if ($Resultado) { //verifica que los datos se envien a la base de datos
